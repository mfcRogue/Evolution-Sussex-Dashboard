...
class MOTRemindersSendByText extends Component
{
    public function render()
    {
          ...
          {
              $mot_text_send = DB::table('customer')
              ->where('Reference', '=', $vehicle_data->CustomerReference)
              ->where('Str1', '<>', '')
              ->where('Email', '=', '')
              ->where('Email2', '=', '')
              ->get();
              foreach($mot_text_send as $ses)
              {
               $count++;
              }
          }
          $mot_text_send = $count;
        return view('livewire.vehicles.count.m-o-t-reminders-send-by-text', ['mot_text_send' => $mot_text_send]);
    }
}
